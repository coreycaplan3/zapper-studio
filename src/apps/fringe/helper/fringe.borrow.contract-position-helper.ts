...
export type CompoundBorrowContractPositionDataProps = FringeSupplyTokenDataProps & {
  supply: number;
  borrow: number;
};
...
@Injectable()
export class FringeBorrowContractPositionHelper {
  constructor(
  ...
  async getPositions({
    ...
    const promisedPositions = appTokens.map(async appToken => {
      ...
      const contractPosition: ContractPosition<FringeSupplyTokenDataProps> = {
